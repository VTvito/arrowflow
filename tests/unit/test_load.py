...
class TestLoadArrowToFormat:
    def test_csv_output(self, sample_arrow_table):
        result = load_arrow_to_format(sample_arrow_table, "csv")
    ...
    def test_json_output(self, sample_arrow_table):
        result = load_arrow_to_format(sample_arrow_table, "json")
        assert isinstance(result, bytes)
    ...
    def test_xlsx_output(self, sample_arrow_table):
        result = load_arrow_to_format(sample_arrow_table, "xlsx")
        assert isinstance(result, bytes)
        assert len(result) > 0
    ...
    def test_parquet_output(self, sample_arrow_table):
        result = load_arrow_to_format(sample_arrow_table, "parquet")
        assert isinstance(result, bytes)
        assert len(result) > 0
